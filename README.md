...
To make this a real review gate, configure the public `release` environment with required reviewers, protect `main` with the `CI / build-and-test` check and at least one PR approval, and protect `v*` tags from update or deletion. A solo repository can keep the manual environment gate, but independent approval requires a trusted collaborator.
